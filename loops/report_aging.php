	...
	<div class="main_data_css">
		...
		<a href='" . $sorturl . "sort_order_pre=ASC&sort=notes_date'><img src='images/sort_asc.png' width='6px' height='12px' style='-webkit-filter:invert(1);filter:invert(1);'></a>&nbsp;<a href='" . $sorturl . "sort_order_pre=DESC&sort=notes_date'><img src='images/sort_desc.png' width='6px' height='12px' style='-webkit-filter:invert(1);filter:invert(1);'></a>
			...
			while ($inv_row = array_shift($inv_res)) {
				...
				$sql = "SELECT trans_status, order_issue,trans_status_update_dt FROM loop_transaction_buyer where id = " . $inv_row["trans_rec_id"];

				$result_comp = db_query($sql);
				$trans_status_update_dt = "";
				while ($row_comp = array_shift($result_comp)) {

					$trans_status_id = $row_comp["trans_status"];

					$order_issue = $row_comp["order_issue"];
					$trans_status_update_dt = $row_comp['trans_status_update_dt'];
				}
				...
				if($trans_status_id == 3 && $trans_status_update_dt < date('Y-m-d H:i:s', strtotime('-14 days')) && $trans_status_update_dt != '0000-00-00 00:00:00' && $trans_nm == "PROMISED TO PAY (P2P): Past Due Customers Who A/R Has Received Confirmation of Payment Being Sent"){
					$classnm = "newtxttheam_withdot_red1";

					$altclass = "newtxttheam_withdot_red1_child";
				}
		...
		<table width="100%">
			...
			<tr>
				...
				<table class="summarytable" width="100%">
					<tr style="background: #2B1B17;">
						<td colspan="8" style="color:#ffffff" align="center"><b>P2P Review Table</b><br>
							As of <?php
									if ($_GET["date_from"] != "") {
										echo date('F d,Y',	 strtotime($_GET["date_from"]));
									} else {
					...
					for ($sl = 0; $sl < $status_length; $sl++) {
						$inv_conunt = 0;
						if ($status_id1[$sl] == 0) {
							$invoice_query = db_query("Select rep_new.* from loop_ar_reptemp_new as rep_new JOIN loop_transaction_buyer as ltb ON ltb.id = rep_new.trans_rec_id where userid = '" . $userid . "' and ((rep_new.trans_status not in (1,2,3,4,99))) and rep_new.inv_total < 0 and ltb.trans_status_update_dt != '0000-00-00 00:00:00' and ltb.trans_status_update_dt < DATE_SUB(NOW(), INTERVAL 14 DAY) ");
						} else {
							$invoice_query = db_query("Select rep_new.* from loop_ar_reptemp_new as rep_new JOIN loop_transaction_buyer as ltb ON ltb.id = rep_new.trans_rec_id where  userid = '" . $userid . "' and rep_new.inv_total < 0 and rep_new.trans_status = '" . $status_id1[$sl] . "' and ltb.trans_status_update_dt != '0000-00-00 00:00:00' and ltb.trans_status_update_dt < DATE_SUB(NOW(), INTERVAL 14 DAY)");
						}
						while ($invoice_row = array_shift($invoice_query)) {
