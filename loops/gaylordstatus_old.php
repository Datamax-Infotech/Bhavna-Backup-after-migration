...
function timestamp_to_date_gy(string $d): string
{
	$da = explode(" ",$d);
	$dp = explode("-", $da[0]);
...
<script language="javascript">
    ...
    function update_details(cnt, comm_log_flg)
	{
		...
		var next_load_date = document.getElementById('ctrl_next_load_available_date'+ cnt).value;
		var note = document.getElementById('note'+ cnt).value;
		var b2b_status = document.getElementById('b2b_status'+ cnt).value;
		
		var compid = 0;	var typeid = ""; var txtmessage = ""; var txtemployee = "";	
		var comm_upd = "";
		...
		document.getElementById("gaylord_div" + cnt).innerHTML = "<br><br>Loading .....<img src='images/wait_animated.gif' />"; 						
		...
		xmlhttp.open("GET","add_gaylorddetails_upd_new.php?comm_upd=" + comm_upd + "&compid=" + compid + "&typeid=" + typeid + "&txtmessage=" + txtmessage + "&txtemployee=" + txtemployee + "&cnt=" +cnt+"&after_actual_inventory="+after_actual_inventory+"&box_id="+box_id+"&expected_loads_per_mo="+expected_loads_per_mo+"&next_load_date="+next_load_date+"&note="+note+"&b2b_status="+b2b_status,true);
		xmlhttp.send();
	}	
	...
	function displayboxdata(colid, sortflg, box_type_cnt)
	{
		...
		xmlhttp.open("GET","gaylord_status_sort_data.php?colid=" + colid + "&sortflg=" + sortflg + "&box_type=" + box_type+ "&owner_dd=" + owner_dd+ "&box_type_cnt=" + box_type_cnt ,true);	
		xmlhttp.send();
	} 
</script>	
...
<div class="main_data_css">
	<div class="dashboard_heading" style="float: left;">
		...
		<span class="tooltiptext">The report allows the user to see all inventory items they own the relationship of (direct ship only though), and allows them to edit the availability of each box. This information will populate the sales matching tools for quoting research.</span></div>
...
<div style="text-align:left;">
	<span style="font-size:18pt;">Box Availability Updater</span><br>
	<i>The Report allows the user to see all inventory items they own the relationship of (direct ship only though), and allows them to edit the availability of each box. This information will populate the sales matching tools for quoting research.</i>
</div>
			...
			<tr>
				...
				<td class="new_text">
					...
					<select name="owner_dd" id="owner_dd">
						<option selected value="all" <?php if(isset($_REQUEST["owner_dd"]) && $_REQUEST["owner_dd"]=="all"){ echo "selected"; } ?>>All</option>
						<option value="0" <?php if(isset($_REQUEST["owner_dd"]) && $_REQUEST["owner_dd"]=="0"){ echo "selected"; } ?> >Operations</option>
						<?php
							db_b2b();
							...
							{
								?>
								<option value="<?php echo $inv_row["employeeID"]; ?>" <?php if(isset($_REQUEST["owner_dd"]) && $_REQUEST["owner_dd"]==$inv_row["employeeID"]){ echo "selected"; } ?>><?php echo $inv_row["name"] ?></option>
								<?php   
							}
						?>  
					</select>
				...
				<td class="new_text">
					<select class="form_component form_select_dd" name="box_type" id="box_type" >
						...
						<option value="Gaylords" <?php if ($_REQUEST["box_type"] == "Gaylords") echo " selected "; ?>>Gaylords</option>
						<option value="Shipping boxes" <?php if ($_REQUEST["box_type"] == "Shipping boxes") echo " selected "; ?>>Shipping boxes</option>
						<option value="Pallets" <?php if ($_REQUEST["box_type"] == "Pallets") echo " selected "; ?>>Pallets</option>
						<option value="Supersacks" <?php if ($_REQUEST["box_type"] == "Supersacks") echo " selected "; ?>>Supersacks</option>
						<option value="Other" <?php if ($_REQUEST["box_type"] == "Other") echo " selected "; ?>>Other</option>
					</select>
				</td>
				<td class="new_text">
	  ...
	  <?php
		...
		//
			...
			foreach ($box_type_str_arr as $box_type_str_arr_tmp){
				...
				if ($box_type_cnt == 5){ $box_type = "Other"; }
				}
				else{
				...
				if(tep_db_num_rows($dt_view_res)>0)
				{
					...
					//
						...
						db();
						$sku = ""; $after_po = 0; $next_load_available_date = ""; $txt_after_po = ""; $expected_loads_per_mo = ""; $b2b_status = "";
						...
						$getEstimatedNextLoad = getEstimatedNextLoad($inv["loops_id"], (int)$warehouse_id, $next_load_available_date, $lead_time, $lead_time, $txt_after_po, (int)$boxes_per_trailer, floatval($expected_loads_per_mo), $st_row["status_key"], 'yes') ;
						...
						if ($inv["vendor_b2b_rescue"] > 0) {
							...
							$comp_nm = get_nickname_val($comp_nm, (int)$comp_b2bid);
							$warehouse = $comp_nm;

							$last_contact_date = ""; $cdate= "";
						...
						//
						//
						//echo $_REQUEST["box_type"] . "- " . $box_type_cnt . "-" . $after_po_val_tmp."-".$estimated_next_load."-/".$inv["box_type"]."---".$supplier_owner_name."<br>";
//'after_actual_inventory' => $inv["after_actual_inventory"]
						if($box_type_cnt==1){
							$gy[]= array('count' => $srno, 'I' => $inv["I"], 'contact_nm' => $contact_nm, 'vendor_b2b_rescue' => $inv["vendor_b2b_rescue"], 'after_actual_inventory' => $txt_after_po,
							'b2b_status' => $inv["b2b_status"], 'vendor_name' => $vendor_name, 'expected_loads_per_mo' => $inv["expected_loads_per_mo"], 'warehouse' => $warehouse, 'loops_id' => $inv["loops_id"], 'V' => $inv["V"], 'loops_id' => $inv["loops_id"], 'description' => $inv["system_description"],
						...
						//
						if($box_type_cnt==2){
							$sb[]= array('count' => $srno, 'I' => $inv["I"], 'contact_nm' => $contact_nm, 'vendor_b2b_rescue' => $inv["vendor_b2b_rescue"], 'after_actual_inventory' => $txt_after_po,
							'b2b_status' => $inv["b2b_status"], 'vendor_name' => $vendor_name, 'expected_loads_per_mo' => $inv["expected_loads_per_mo"], 'warehouse' => $warehouse, 'loops_id' => $inv["loops_id"], 'V' => $inv["V"], 'loops_id' => $inv["loops_id"], 'description' => $inv["system_description"],
							'availability' => $inv["availability"], 'N' => $inv["N"], 'DT' => $inv["DT"], 'boxes_pertrailer' => $boxes_pertrailer, 'b2b_fob' => $b2b_fob, 'supplier_owner_name' => $supplier_owner_name,'next_load_available_date' => $next_load_available_date,'next_load_available_date_display' => $next_load_available_date_display, 'buynowshipin' => $buynowshipin, 'estimated_next_load' => $estimated_next_load, 'load_date_col_color' => $load_date_col_color, 'sales_order_qty' => $sales_order_qty);
						}
						//
						if($box_type_cnt==3){
							$pal[]= array('count' => $srno, 'I' => $inv["I"], 'contact_nm' => $contact_nm, 'vendor_b2b_rescue' => $inv["vendor_b2b_rescue"], 'after_actual_inventory' => $txt_after_po,
							'b2b_status' => $inv["b2b_status"], 'vendor_name' => $vendor_name, 'expected_loads_per_mo' => $inv["expected_loads_per_mo"], 'warehouse' => $warehouse, 'loops_id' => $inv["loops_id"], 'V' => $inv["V"], 'loops_id' => $inv["loops_id"], 'description' => $inv["system_description"],
							'availability' => $inv["availability"], 'N' => $inv["N"], 'DT' => $inv["DT"], 'boxes_pertrailer' => $boxes_pertrailer, 'b2b_fob' => $b2b_fob, 'supplier_owner_name' => $supplier_owner_name,'next_load_available_date' => $next_load_available_date,'next_load_available_date_display' => $next_load_available_date_display, 'buynowshipin' => $buynowshipin, 'estimated_next_load' => $estimated_next_load, 'load_date_col_color' => $load_date_col_color, 'sales_order_qty' => $sales_order_qty);
						}
						//
						if($box_type_cnt==4){
							$sup[]= array('count' => $srno, 'I' => $inv["I"], 'contact_nm' => $contact_nm, 'vendor_b2b_rescue' => $inv["vendor_b2b_rescue"], 'after_actual_inventory' => $txt_after_po,
							'b2b_status' => $inv["b2b_status"], 'vendor_name' => $vendor_name, 'expected_loads_per_mo' => $inv["expected_loads_per_mo"], 'warehouse' => $warehouse, 'loops_id' => $inv["loops_id"], 'V' => $inv["V"], 'loops_id' => $inv["loops_id"], 'description' => $inv["system_description"],
							'availability' => $inv["availability"], 'N' => $inv["N"], 'DT' => $inv["DT"], 'boxes_pertrailer' => $boxes_pertrailer, 'b2b_fob' => $b2b_fob, 'supplier_owner_name' => $supplier_owner_name,'next_load_available_date' => $next_load_available_date,'next_load_available_date_display' => $next_load_available_date_display, 'buynowshipin' => $buynowshipin, 'estimated_next_load' => $estimated_next_load, 'load_date_col_color' => $load_date_col_color, 'sales_order_qty' => $sales_order_qty);
						}
						if($box_type_cnt==5){
							$other[]= array('count' => $srno, 'I' => $inv["I"], 'contact_nm' => $contact_nm, 'vendor_b2b_rescue' => $inv["vendor_b2b_rescue"], 'after_actual_inventory' => $txt_after_po,
							'b2b_status' => $inv["b2b_status"], 'vendor_name' => $vendor_name, 'expected_loads_per_mo' => $inv["expected_loads_per_mo"], 'warehouse' => $warehouse, 'loops_id' => $inv["loops_id"], 'V' => $inv["V"], 'loops_id' => $inv["loops_id"], 'description' => $inv["system_description"],
							'availability' => $inv["availability"], 'N' => $inv["N"], 'DT' => $inv["DT"], 'boxes_pertrailer' => $boxes_pertrailer, 'b2b_fob' => $b2b_fob, 'supplier_owner_name' => $supplier_owner_name,'next_load_available_date' => $next_load_available_date,'next_load_available_date_display' => $next_load_available_date_display, 'buynowshipin' => $buynowshipin, 'estimated_next_load' => $estimated_next_load, 'load_date_col_color' => $load_date_col_color, 'sales_order_qty' => $sales_order_qty);
						}
						//
	...
	foreach($box_name_arr as $box_name){
		...
		if($total_rec_act > 0)
		{
							?>
	<tr align="middle">
			<td colspan="16" class="section_heading" style="height: 16px"><strong>Box Availability - <?php echo $boxtype;?></strong></td>
		</tr>
		<tr><td>
		<div id="btype<?php echo $boxtype_cnt; ?>">
			<table width="100%" cellspacing="1" cellpadding="2"> 
		<tr vAlign="center">
			<td class="section_heading" width="50px"><a href='gaylordstatus.php?sort=count&sort_order_pre=<?php echo $sort_order_pre;?>'>#</a></td>
			<td class="section_heading" width="50px"><strong>Rep</strong>
			 <font size="1" face="Arial, Helvetica, sans-serif" color="#333333">&nbsp;<a href="javascript:void();" onclick="displayboxdata(1,1,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_asc.jpg" width="5px;" height="10px;"></a>&nbsp;<a href="javascript:void();" onclick="displayboxdata(1,2,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_desc.jpg"  width="5px;" height="10px;"></a>
             </font> 
			</td>
			<td class="section_heading" width="50px"><strong>Orders</strong></td>
			<td class="section_heading" width="100px"><strong>Qty Avail Now</strong>
			 <font size="1" face="Arial, Helvetica, sans-serif" color="#333333">&nbsp;<a href="javascript:void();" onclick="displayboxdata(2,1,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_asc.jpg" width="5px;" height="10px;"></a>&nbsp;<a href="javascript:void();" onclick="displayboxdata(2,2,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_desc.jpg"  width="5px;" height="10px;"></a>
             </font> 
			</td>
			<td class="section_heading" width="100px"><strong>Expected # Loads/Month</strong>
			<font size="1" face="Arial, Helvetica, sans-serif" color="#333333">&nbsp;<a href="javascript:void();" onclick="displayboxdata(3,1,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_asc.jpg" width="5px;" height="10px;"></a>&nbsp;<a href="javascript:void();" onclick="displayboxdata(3,2,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_desc.jpg"  width="5px;" height="10px;"></a>
             </font>
			</td>
			<td class="section_heading" width="100px"><strong>Next Load Avail Date</strong>
			<font size="1" face="Arial, Helvetica, sans-serif" color="#333333">&nbsp;<a href="javascript:void();" onclick="displayboxdata(4,1,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_asc.jpg" width="5px;" height="10px;"></a>&nbsp;<a href="javascript:void();" onclick="displayboxdata(4,2,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_desc.jpg"  width="5px;" height="10px;"></a>
             </font>
			</td>
			<td class="section_heading" width="100px"><strong>Buy Now, Load Can Ship In</strong>
			<font size="1" face="Arial, Helvetica, sans-serif" color="#333333">&nbsp;<a href="javascript:void();" onclick="displayboxdata(5,1,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_asc.jpg" width="5px;" height="10px;"></a>&nbsp;<a href="javascript:void();" onclick="displayboxdata(5,2,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_desc.jpg"  width="5px;" height="10px;"></a>
             </font>
			</td>
			<td class="section_heading" width="100px"><strong>Per Truckload</strong>
			<font size="1" face="Arial, Helvetica, sans-serif" color="#333333">&nbsp;<a href="javascript:void();" onclick="displayboxdata(6,1,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_asc.jpg" width="5px;" height="10px;"></a>&nbsp;<a href="javascript:void();" onclick="displayboxdata(6,2,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_desc.jpg"  width="5px;" height="10px;"></a>
             </font>
			</td>
			<td class="section_heading" width="100px"><strong>Min FOB</strong><br>
			<font size="1" face="Arial, Helvetica, sans-serif" color="#333333">&nbsp;<a href="javascript:void();" onclick="displayboxdata(7,1,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_asc.jpg" width="5px;" height="10px;"></a>&nbsp;<a href="javascript:void();" onclick="displayboxdata(7,2,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_desc.jpg"  width="5px;" height="10px;"></a>
             </font>
			</td>
			<td class="section_heading" width="100px"><strong>B2B ID</strong><br>
			<font size="1" face="Arial, Helvetica, sans-serif" color="#333333">&nbsp;<a href="javascript:void();" onclick="displayboxdata(8,1,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_asc.jpg" width="5px;" height="10px;"></a>&nbsp;<a href="javascript:void();" onclick="displayboxdata(8,2,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_desc.jpg"  width="5px;" height="10px;"></a>
             </font>
			</td>
			<td class="section_heading" width="155px"><strong>Supplier</strong><br>
			<font size="1" face="Arial, Helvetica, sans-serif" color="#333333">&nbsp;<a href="javascript:void();" onclick="displayboxdata(9,1,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_asc.jpg" width="5px;" height="10px;"></a>&nbsp;<a href="javascript:void();" onclick="displayboxdata(9,2,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_desc.jpg"  width="5px;" height="10px;"></a>
             </font>
			</td>	  
			<td class="section_heading" width="150px" ><strong>Description</strong>
			<font size="1" face="Arial, Helvetica, sans-serif" color="#333333">&nbsp;<a href="javascript:void();" onclick="displayboxdata(10,1,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_asc.jpg" width="5px;" height="10px;"></a>&nbsp;<a href="javascript:void();" onclick="displayboxdata(10,2,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_desc.jpg"  width="5px;" height="10px;"></a>
             </font>
			</td>	
			<td class="section_heading" width="50px"><strong>B2B Status</strong>
				<font size="1" face="Arial, Helvetica, sans-serif" color="#333333">&nbsp;<a href="javascript:void();" onclick="displayboxdata(11,1,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_asc.jpg" width="5px;" height="10px;"></a>&nbsp;<a href="javascript:void();" onclick="displayboxdata(11,2,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_desc.jpg"  width="5px;" height="10px;"></a>
             </font>
			</td>	  	
			<td class="section_heading" width="150px" >Notes
				<font size="1" face="Arial, Helvetica, sans-serif" color="#333333">&nbsp;<a href="javascript:void();" onclick="displayboxdata(12,1,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_asc.jpg" width="5px;" height="10px;"></a>&nbsp;<a href="javascript:void();" onclick="displayboxdata(12,2,<?php echo $boxtype_cnt; ?>);" ><img src="images/sort_desc.jpg"  width="5px;" height="10px;"></a>
				</font>
			</td>	  
			<td class="section_heading" width="100px" >Communication Log</td>	  
			<td class="section_heading" width="50px">Update</td>	
		</tr>
			<?php
			$cellClr = '';$cellClrQAN = '';
			...
			<tr id="gaylord_div<?php echo $srno;?>" vAlign="center">
				...
				<td class="form_label" width="50px"><font size=1><?php echo $srno; ?></font>
				<input type=hidden name="box_id" id="box_id<?php echo $srno;?>" value="<?php echo  $MGArraytmp2["I"];?>">
			</td>	

			<td class="form_label" width="100px"><?php echo $MGArraytmp2["supplier_owner_name"];?></td>

			<td class="form_label" width="50px"><?php echo $order_col_color; ?>
			  <?php 
			  if ($MGArraytmp2["sales_order_qty"] > 0) {?>
				<div onclick="display_preoder_sel(<?php echo $srno;?>, <?php echo $MGArraytmp2["sales_order_qty"];?>, <?php echo $MGArraytmp2["loops_id"];?>, <?php echo $MGArraytmp2["vendor_b2b_rescue"];?>)" style="cursor: pointer;">
				 <u>View</u></div>
				<?php
										}
			else{
			...
			<td class="form_label" width="100px" bgcolor="<?php echo $cellClrQAN;?>">
				<input type="text" id="after_actual_inventory<?php echo $srno;?>" name="after_actual_inventory" value="<?php echo $MGArraytmp2["after_actual_inventory"]; ?>" size="5"> 
			</td>
			<td class="form_label" width="100px"><input type="text" id="expected_loads_per_mo<?php echo $srno;?>" name="expected_loads_per_mo"  value="<?php echo $MGArraytmp2["expected_loads_per_mo"]; ?>" size="5"></td>
			<?php
			$cellClr = '';
			...
			<td class="form_label" width="100px"  bgcolor="<?php echo $cellClr;?>">
				...
				<input size="10" type="text" id="ctrl_next_load_available_date<?php echo $srno;?>" name="ctrl_next_load_available_date"  value="<?php echo $MGArraytmp2["next_load_available_date_display"]; ?>">
				<a href="#" onclick="cal_load_available_date.select(document.frm_main.ctrl_next_load_available_date<?php echo $srno;?>,'anchorload_available_date<?php echo $srno;?>','yyyy-MM-dd'); return false;" name="anchorload_available_date" id="anchorload_available_date<?php echo $srno;?>"><img border="0" src="images/calendar.jpg"></a>
			...
			<td class="form_label" width="100px"><?php echo $MGArraytmp2["estimated_next_load"]; ?></td>
			<td class="form_label" width="100px"><?php echo $MGArraytmp2["boxes_pertrailer"]; ?></td>
			<td class="form_label" width="100px"><?php echo $MGArraytmp2["b2b_fob"]; ?></td>
			<td class="form_label" width="100px"><?php echo $MGArraytmp2["I"]; ?></td>
			<?php 
			  $contact_nm = "";	$comm_log_color = "<font size=1>"; 
			...
			<?php } else { 
				...
				$warehouse = $vendor_name;
			?>				
				<td class="form_label" width="150px"><font size=1><a href="addVendor.php?act=edit&id=<?php echo $MGArraytmp2["V"]; ?>" target="_blank"><?php echo $vendor_name; ?></a></font></td>	
			<?php } ?>						

			<td class="form_label" width="200px"><font size=1><a href="manage_box_b2bloop.php?id=<?php echo $MGArraytmp2["loops_id"]; ?>&proc=Edit&" target="_blank"><?php echo $MGArraytmp2["description"];?></a></td>
			
			<td class="form_label" width="50px"><font size=1>
				<select name="b2b_status" id="b2b_status<?php echo $srno;?>" style="width:150px;">
					...
					?>
						<option <?php if ($st_row["status_key"] == $MGArraytmp2["b2b_status"] ) echo " selected "; ?>  value="<?php echo $st_row["status_key"]; ?>"><?php echo $st_row["box_status"]; ?></option>
					<?php
					}
					?>
				</select>			
			...
			<td class="form_label" width="150px"><textarea rows=2 cols=20 name="note" onkeypress='return chkcharacter(event);' id="note<?php echo $srno;?>"><?php echo $MGArraytmp2["N"]; ?></textarea>
			<br><?php echo $notes_date_col_color; if ($MGArraytmp2["DT"] != "") echo timestamp_to_date_gy($MGArraytmp2["DT"]); ?></font>
			...
			<td class="form_label" width="100px">
				...
				<table width="96%" border="0" cellspacing="1" cellpadding="1" class="sub_tables_s">
					<tr valign="top">
						<td >
							<input type=hidden name="companyID" id="companyID<?php echo $srno;?>" value="<?php echo $comp_b2bid?>">
							<input type=hidden name="loopID" id="loopID<?php echo $srno;?>" value="<?php echo $MGArraytmp2["vendor_b2b_rescue"]?>">
							...
							<textarea rows="5" name="message" id="message<?php echo $srno;?>" cols="15"></textarea>
				...
				<td class="form_label" width="50px"><input type="button" value="Update" onclick="update_details(<?php echo $srno; ?>,2)"></td>
