...
<?php
$query = "SELECT loop_transaction_buyer.so_date AS A, loop_transaction_buyer.so_employee  AS employee,  loop_warehouse.warehouse_name AS warehousename,  loop_transaction_buyer.warehouse_id AS wid ,  loop_transaction_buyer.id AS trid, loop_transaction_buyer.picklist_print , loop_transaction_buyer.picklist_printtime  
FROM loop_transaction_buyer INNER JOIN loop_warehouse ON loop_transaction_buyer.warehouse_id = loop_warehouse.id 
WHERE loop_transaction_buyer.ignore = 0  AND loop_transaction_buyer.shipped = 0 
AND loop_transaction_buyer.so_entered = 1 AND loop_transaction_buyer.bol_signed_uploaded = 0  AND loop_transaction_buyer.id > 67 order by trid ASC" ;
//echo $query;
$res = db_query($query);
...
{
	$query2 = "SELECT * FROM loop_salesorders WHERE location_warehouse_id = 4146 AND trans_rec_id = " . $row["trid"] . " GROUP BY trans_rec_id";
	...
	?>
		...
		<tr vAlign="center">
			...
			<td bgColor="<?php echo $bg;?>" class="style3"  align="center">
				...
				<p align="center">	
				<span class="infotxt"><u><a href="http://loops.usedcardboardboxes.com/search_results.php?warehouse_id=<?php echo $row["wid"]?>&rec_type=Supplier&proc=View&searchcrit=&id=<?php echo $row["wid"]?>&rec_id=<?php echo $row["trid"];?>&display=buyer_ship"><?php echo $row["warehousename"];?></a></u>
