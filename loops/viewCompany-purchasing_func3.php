		...
		function todoitem_edit(unqid, compid) {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					parent.document.getElementById("light_reminder_todo").innerHTML = "<a href='javascript:void(0)' onclick=document.getElementById('light_reminder_todo').style.display='none';document.getElementById('fade_todo').style.display='none'>Close</a> &nbsp;<center></center><br/>" + xmlhttp.responseText;
					parent.document.getElementById('light_reminder_todo').style.display = 'block';

					parent.document.getElementById('light_reminder_todo').style.left = (n_left + 30) + 'px';
					parent.document.getElementById('light_reminder_todo').style.top = n_top + 200 + 'px';
					parent.document.getElementById('light_reminder_todo').style.width = 700 + 'px';
					parent.document.getElementById('light_reminder_todo').style.height = 200 + 'px';
	...
	<table width="100%">
		<tr>
			<td valign="top">
				...
				<table width="550" border="0" cellspacing="0" cellpadding="0" class="bg_color_e4e4e4">
					<tr align="center">
					...
					<?php if (($row["haveNeed"] == "Need Boxes") || ($row["haveNeed"] == "Looking / Have Boxes") || ($row["haveNeed"] == "Have Boxes")) { ?>
						...
						<tr>
							<td valign="middle" align="left">
								...
								<?php
								// Convert the 'next_date' to a timestamp if it is set and not empty
								$start_date = isset($row["next_date"]) && $row["next_date"] != "" ? strtotime($row["next_date"]) : '';
								?>
								<input type="text" name="start_date" id="start_date" size="11" value="<?php echo $start_date ? date('m/d/Y', $start_date) : ''; ?>">
								<a style='color:#0000FF;' href="#" onclick="cal1xx.select(document.nextstep.start_date,'anchor1xx','MM/dd/yyyy'); return false;" name="anchor1xx" id="anchor1xx">
				...
				<?php if ($crm_trans_tbl_flg == "yes") {  ?>
					...
					<table width='550' border='0' cellspacing='2' cellpadding='2' bgcolor='#E4E4E4'>
						<br>
										...
										<tr>
											<td class="bg_color_e4e4e4" colspan="4">
												<div id="div_crm_comp_email" style="display:none;">
													...
													$ship_email_contact_ar = "";
													$ship_email_contact_array1 = "";
													$email_contact_array1 = "";
													...
													if (($billto_email_array_cnt > 0) || ($sellto_email_array_cnt > 0)) {
														if ($billto_email_array_cnt > 0 && !empty($email_array)) {
															$email_list = array_unique(array_merge($email_array, $billto_email_array));
														}
														if ($sellto_email_array_cnt > 0 && !empty($email_array)) {
