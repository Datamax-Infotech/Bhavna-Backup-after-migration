...
<head>
    <title>Pallet Leaderboard Reports</title>
...
<body>
    ...
    function leadertbl_purchasing(
        ...
        string $ttylyesno,
        string $in_dt_range,
        string|null $activity_tracker_flg = "no"
    ): void {
        // if ($activity_tracker_flg == null) {
        //     $activity_tracker_flg = "no";
        // }
        ...
        while ($rowemp = array_shift($result)) {
                ...
                while ($rowemp_empq = array_shift($result_empq)) {
                    ...
                    if ($donot_add == "") {
                        if ($rowemp_empq["quota_month"] <= $dt_month_value_1) {
                            if ($days_in_month > 0 && $rowemp_empq["quota_month"] == $dt_month_value_1) {
                                $donot_add = "yes";
            ...
            if ($str_box_list_ids != "") {
                ...
                while ($row_rs_tmprs = array_shift($qry)) {
                    ...
                    while ($box_row = array_shift($box_res)) {
                        ...
                        $boxdesc = round(floatval($box_row["blength"])) . " ";
                        if ($box_row["blength_frac"] != "")
                            $boxdesc .= $box_row["blength_frac"] . " ";
                        $boxdesc .= "x " . round(floatval($box_row["bwidth"])) . " ";
                        if ($box_row["bwidth_frac"] != "")
                            $boxdesc .= $box_row["bwidth_frac"] . " ";
                        $boxdesc .= "x " . round(floatval($box_row["bdepth"])) . " ";
                    ...
                    while ($row_rs_data_main = array_shift($qry_box_main)) {
                        ...
                        $total_val =  number_format((float)str_replace(",", "", $row_rs_data_main["total"]), 2);
                        $total = $total + str_replace(",", "", $total_val);
                ...
                if ($str_box_list_ids != "") {
                    ...
                    while ($row_rs_tmprs = array_shift($qry)) {
                        ...
                        while ($row_rs_data_main = array_shift($qry_box_main)) {
                            $quantity = $quantity + $row_rs_data_main["quantity"];
                            $price = $row_rs_data_main["price"];
                            $total_val =  number_format((float)str_replace(",", "", $row_rs_data_main["total"]), 0);
                            $total = $total + str_replace(",", "", $total_val);
                            $box_desc = $row_rs_data_main['description'];
                            $invoice_amt_ind = $invoice_amt_ind + $row_rs_data_main["quantity"] * $row_rs_data_main["price"];
                        }
            ...
            if ($headtxt == "LAST YEAR") {
                ...
                if ($str_box_list_ids != "") {
                    ...
                    $tmp_trans_id = 0;
                    $vendor_b2b_rescue = 0;
                    $qry = db_query("Select box_id, qty, loop_bol_tracking.trans_rec_id, loop_invoice_details.total as loop_inv_amount FROM loop_bol_tracking inner join loop_invoice_details on loop_invoice_details.trans_rec_id = loop_bol_tracking.trans_rec_id where loop_invoice_details.timestamp between '" . $start_Dt_lasty . "'  AND '" . $end_Dt_lasty . " 23:59:59'  and loop_invoice_details.trans_rec_id in (" . $str_box_list_transids . ") and box_id in (" . $str_box_list_ids . ")");
                    while ($row_rs_tmprs = array_shift($qry)) {
                        ...
                        if ($quantity > 0) {
                            ...
                            $profit_val_p = 0;
                            if ($invoice_amt > 0) {
                                $profit_val_p = (($gross_profit_val * 100) / (float)$invoice_amt);
                            }
							...
							<td bgColor='#E4EAEB'>" . number_format($price, 2) . "</td>
							<td bgColor='#E4EAEB' align='right'>$" . number_format((float)str_replace(",", "", $total), 0) . "</td>
							<td bgColor='#E4EAEB' $profit_val_str align='right'>" . (float)$profit_val . "</td>
							<td bgColor='#E4EAEB' $profit_val_str >" . (float)$profit_val_per . "</td>
							</tr>";
                            }
                        }
                        $tmp_trans_id = $row_rs_tmprs["trans_rec_id"];
            ...
            if ($show_data == "yes") {
                ...
                $MGArray[] = array(
                    ...
                    'b2bempid' => $rowemp["b2b_id"],
                    'name_initial' => $rowemp["initials"],
                    'emp_initials_list' => $emp_initials_list,
                    'emp_b2bid_list' => $emp_b2bid_list,
                    'emp_id_list' => $emp_id_list,
                    'emp_eml_list' => $emp_eml_list,
                    'empid' => $rowemp["id"],
                    'start_Dt' => $start_Dt,
                    'end_Dt' => $end_Dt,
                    'leaderboard' => $rowemp["leaderboard"],
                    'deal_count' => $summtd_dealcnt,
                    'quota' => $quota_in_st_en,
                    'profit_val_org' => $profit_val_org,
                    'quote_amount_tweek' => $quote_amount_tweek,
                    'quotatodate' => $monthly_qtd,
                    'po_entered' => $summtd_SUMPO,
                    'ttly' => $profit_val_org_ttly,
                    'percent_val' => $color,
                    'rev_lastyr_tilldt' => $profit_val_org_rev_lastyr,
                    'lisoftrans' => $lisoftrans,
                    'lisoftrans_ttly' => $lisoftrans_ttly,
                    'lisoftrans_lastyr' => $lisoftrans_lastyr
                );
            }
        }
        ...
        foreach ($MGArray as $MGArraytmp2) {
            ...
            if (($headtxt == "THIS WEEK" || $headtxt == "LAST WEEK") && ($po_flg == "no")) {
                ...
                if ($str_box_list_ids != "") {
                    ...
                    while ($row_rs_tmprs = array_shift($qry)) {
                        ...
                        $lisoftrans_detail_list .= "<tr><td bgColor='$tbl_color'><a target='_blank' href='viewCompany.php?ID=" . $b2bid . "&show=transactions&warehouse_id=" . $wid . "&rec_type=Supplier&proc=View&searchcrit=&id=" . $wid . "&rec_id=" . $row_rs_tmprs["trans_rec_id"] . "&display=buyer_view'>" . $row_rs_tmprs["trans_rec_id"] . "</a></td><td bgColor='$tbl_color'>" . $nickname . "</td><td bgColor='$tbl_color'>" . $po_date . "</td><td bgColor='$tbl_color'>" . $name . "</td><td bgColor='$tbl_color' align='right'>$" . number_format((float)str_replace(",", "", $gr_total), 0) . "</td></tr>";
            ...
            if ($headtxt == "PALLET Leaderboard") {
                ...
                $per_val = 0;
                if ($sales_revenue > 0) {
                    $summtd_SUMPO_numeric = (float) str_replace(",", "", number_format($summtd_SUMPO, 0));
                    $sales_revenue_numeric = (float) str_replace(",", "", number_format($sales_revenue, 0));
                    $per_val = number_format(($summtd_SUMPO_numeric * 100) / $sales_revenue_numeric, 2);
                }
                if ($per_val >= 20) {
                    $per_val_color = "green";
                } else {
                ...
                $first_time_rec = 0;
                if ($MGArraytmp2["leaderboard"] == 0 && $MGArraytmp2["empid"] == 0) {
                    $result_crm = db_query("SELECT 1 as cnt FROM loop_transaction inner join loop_boxes_sort on loop_boxes_sort.trans_rec_id = loop_transaction.id inner join loop_boxes on loop_boxes.id = loop_boxes_sort.box_id WHERE loop_transaction.id in (SELECT min(id) FROM loop_transaction group by warehouse_id) and loop_transaction.ignore < 1 and loop_boxes.owner not in (" . $emp_id_list . ") AND STR_TO_DATE(loop_boxes_sort.sort_date, '%m/%d/%Y') between '" . $MGArraytmp2["start_Dt"] . "' AND '" . $MGArraytmp2["end_Dt"] . " 23:59:59' group by loop_boxes_sort.trans_rec_id");
                } else {
                    $result_crm = db_query("SELECT 1 as cnt FROM loop_transaction inner join loop_boxes_sort on loop_boxes_sort.trans_rec_id = loop_transaction.id inner join loop_boxes on loop_boxes.id = loop_boxes_sort.box_id WHERE loop_transaction.id in (SELECT min(id) FROM loop_transaction group by warehouse_id) and loop_transaction.ignore < 1 and loop_boxes.owner = '" . $MGArraytmp2["empid"] . "' AND STR_TO_DATE(loop_boxes_sort.sort_date, '%m/%d/%Y') between '" . $MGArraytmp2["start_Dt"] . "' AND '" . $MGArraytmp2["end_Dt"] . " 23:59:59' group by loop_boxes_sort.trans_rec_id");
                }
                ...
                $tot_quota_quotes = $tot_quota_quotes + $no_of_pos_cnt;

                $tot_quote_amount = $tot_quote_amount + $quote_amount;
                db();
                ...
                if ($headtxt == "LAST WEEK" || $headtxt == "LAST MONTH" || $headtxt == "LAST QUARTER" || $headtxt == "LAST YEAR") {
                    if ($monthly_qtd > 0 && ($summtd_SUMPO * 100 / $monthly_qtd) >= 100) {
                        $color_y_new = "green";
                    } elseif ($monthly_qtd > 0 && ($summtd_SUMPO * 100 / $monthly_qtd) < 100 && $summtd_SUMPO > 0) {
                        $color_y_new = "red";
                    ...
                    if (($monthly_qtd_TD > 0) && ($summtd_SUMPO * 100 / $monthly_qtd_TD) >= 100) {
                        $color_y_new = "green";
                    } elseif (($monthly_qtd_TD > 0) && ($summtd_SUMPO * 100 / $monthly_qtd_TD) < 100 && $summtd_SUMPO > 0 && $monthly_qtd_TD > 0) {
                        $color_y_new = "red";
            ...
            while ($row_rs_tmprs = array_shift($qry)) {
                ...
                if ($quantity > 0) {
                    ...
                    if ($double_checked == 1) {
                        ...
                        if ($invoice_amt > 0) {
                            $profit_val_p = (($gross_profit_val * 100) / (float)$invoice_amt);
                            // $profit_val_p = (($gross_profit_val * 100) / (float)str_replace(",", "", $invoice_amt));
        ...
        $global_lisoftrans_ttly = ($lisoftrans_ttly);
        //echo "</table>";
    }
    ...
    function leadertbl(
        ...
        $emp_eml_list = rtrim($emp_eml_list, ",");
        if ($activity_tracker_flg == "yes") {
            $sql = "SELECT id, name, initials, email, b2b_id, leaderboard  FROM loop_employees WHERE activity_tracker_flg_pallet = 1 and status = 'Active' union SELECT 0 as id, 'Other' as name, 'OT' as initials, '' as email, 0 as b2b_id, 0 leaderboard FROM loop_employees WHERE id = 1";
        } elseif ($po_flg == "yes") {
            $sql = "SELECT id, name, initials, email, b2b_id, leaderboard  FROM loop_employees WHERE pallet_leaderboard = 1 and status = 'Active' union SELECT 0 as id, 'Other' as name, 'OT' as initials, '' as email, 0 as b2b_id, 0 leaderboard FROM loop_employees WHERE id = 1";
        ...
        while ($rowemp = array_shift($result)) {
			...
			while ($summtd_finalpmt = array_shift($result_finalpmt)) {
                ...
                $estimated_cost = 0;
                if ($po_flg != "yes") {
                    $qryB2bCogs = "SELECT loop_invoice_details.timestamp, loop_transaction_buyer.id, loop_transaction_buyer.warehouse_id, loop_transaction_buyer.UCBZeroWaste_flg, loop_transaction_buyer.inv_date_of, sum(loop_transaction_buyer_payments.estimated_cost) as estimated_cost, loop_transaction_buyer.Leaderboard FROM loop_transaction_buyer LEFT JOIN loop_invoice_details ON loop_invoice_details.trans_rec_id = loop_transaction_buyer.id INNER JOIN loop_transaction_buyer_payments ON loop_transaction_buyer_payments.transaction_buyer_id = loop_transaction_buyer.id WHERE loop_transaction_buyer.Leaderboard = 'PALLETS' and loop_transaction_buyer.id = '" . $summtd["id"] . "'  and loop_transaction_buyer.ignore = 0 group by loop_transaction_buyer.id ORDER BY loop_transaction_buyer.id";
                    db();
                ...
                <td bgColor='#E4EAEB' align='right'>$" . number_format(($inv_amt_totake - $estimated_cost), 0) . "</td><td bgColor='#E4EAEB' align='right'>" . number_format(($inv_amt_totake !== 0 ? ($inv_amt_totake - $estimated_cost) * 100 / (float)str_replace(",", "", number_format($inv_amt_totake, 0)) : 0), 2) . "%</td></tr>";
                }
            ...
            if ($summtd_SUMPO > 0) {
				...
				<td bgColor='#ABC5DF' align='right'>" . number_format($summtd_SUMPO * 100 / (float)str_replace(",", "", number_format($summtd_SUMPO_sale_rev, 0)), 2) . "%</td></tr>";
                }
            }
            $lisoftrans .= "</table></span>";
            ...
            if ($ttylyesno == "ttylyes") {
                ...
                if ($summ_ttly > 0) {
					...
					<td bgColor='#ABC5DF' align='right'>" . number_format($summ_ttly * 100 / (float)str_replace(",", "", number_format($summ_ttly_sales_rev, 0)), 2) . "%</td></tr>";
            ...
            if ($add_entry == "yes") {
                $MGArray[] = array(
                    ...
                    'b2bempid' => $rowemp["b2b_id"],
                    'leaderboard' => $rowemp["leaderboard"],
                    'summtd_SUMPO_activity' => $summtd_SUMPO_activity,
                    'empid' => $rowemp["id"],
                    'start_Dt' => $start_Dt,
                    'end_Dt' => $end_Dt,
                    'deal_count' => $summtd_dealcnt,
                    ...
                    'quotatodate' => $monthly_qtd,
                    'po_entered' => $summtd_SUMPO,
                    'summtd_SUMPO_sale_rev' => $summtd_SUMPO_sale_rev,
                    'po_entered_other_tweek' => $summtd_SUMPO_tweek,
                    'ttly' => $summ_ttly,
                    'percent_val' => $color,
                    'rev_lastyr_tilldt' => $rev_lastyr_tilldt,
                    'lisoftrans' => $lisoftrans,
                    'lisoftrans_ttly' => $lisoftrans_ttly,
                    'lisoftrans_lastyear' => $lisoftrans_lastyear,
                    'lisoftrans_for_total' => $lisoftrans_for_total
                );
            }
        }
        ...
        foreach ($MGArray as $MGArraytmp2) {
            ...
            if ($headtxt == "PALLET Leaderboard") {
                echo "<tr><td bgColor='$tbl_color' width='260px' align ='left'>" . $name . "</td>";
                echo "<td bgCyolor='$tbl_color' align ='right'>$" . number_format($summtd_SUMPO_sale_rev, 0) . "</td>";
                ...
                if ($summtd_SUMPO_sale_rev > 0 && $summtd_SUMPO > 0) {
                    $profit_mrg = number_format((float)str_replace(",", "", number_format($summtd_SUMPO, 0)) * 100 / (float)str_replace(",", "", number_format($summtd_SUMPO_sale_rev, 0)), 2);
                }
                if ($profit_mrg >= 20) {
                ...
                if ($profit_mrg != "") {
                    ...
                    // Convert profit margin to float
                    $profit_mrg_numeric = (float)$profit_mrg;

                    // Output the formatted profit margin
                    echo "<td bgColor='$tbl_color' align='right'><font color='" . $profit_mrg_color . "'>" . number_format($profit_mrg_numeric, 2) . "%</font></td>";
                } else {
                    echo "<td bgColor='$tbl_color' align = 'right'></td>";
                }
            ...
            {*/
                ...
                if ($headtxt == "LAST WEEK" || $headtxt == "LAST MONTH" || $headtxt == "LAST QUARTER" || $headtxt == "LAST YEAR") {
                    ...
                    if ($monthly_qtd_TD > 0 && ($summtd_SUMPO * 100 / $monthly_qtd_TD) >= 100) {
                        $color_y_new = "green";
                    } elseif (($monthly_qtd_TD > 0 && ($summtd_SUMPO * 100 / $monthly_qtd_TD) < 100) || $summtd_SUMPO < 0) {
                        $color_y_new = "red";
                    } elseif ($summtd_SUMPO > 0 && $monthly_qtd_TD == 0) {
                        $color_y_new = "green";
                    } elseif ($summtd_SUMPO == 0 && $monthly_qtd_TD > 0) {
        ...
        while ($summtd = array_shift($resultmtd)) {
            ...
            if ($po_flg == "yes") {
                ...
                <td bgColor='#E4EAEB' align='right'>" . number_format(($inv_amt_totake !== 0 ? (float)str_replace(",", "", number_format(($inv_amt_totake - $estimated_cost), 0)) * 100 / (float)str_replace(",", "", number_format($inv_amt_totake, 0)) : 0), 0) . "%</td></tr>";
        ...
        if ($headtxt == "THIS QUARTER" || $headtxt == "LAST QUARTER" || $headtxt == "THIS QUARTER LAST YEAR") {
            ...
            $quota_in_st_en = $quota;
            if ($headtxt == "LAST QUARTER") {
                $monthly_qtd_b2b_rev = ($days_today * $quota) / 91;
            } else {
        ...
        if ($headtxt == "THIS YEAR" || $headtxt == "LAST YEAR" || $headtxt == "LAST TO LAST YEAR") {
            ...
            if ($headtxt == "LAST YEAR") {
                ...
                $monthly_qtd_b2b_rev = ($days_today * $quota) / 365;
            } else {
                $monthly_qtd_b2b_rev = $quota_mtd;
            }
        }
        ...
        if ($headtxt == "PALLET Leaderboard") {
            ...
            if ($tot_quotaactual_mtd > 0 && $revenue_sales_b2b > 0) {
                ...
                $quota_actual = (float)str_replace(",", "", $tot_quotaactual_mtd);
                $revenue_sales = (float)str_replace(",", "", $revenue_sales_b2b);

                // Calculate percentage if revenue_sales is not zero
                if ($revenue_sales != 0) {
                    $per_val = number_format(($quota_actual * 100) / $revenue_sales, 2);
                }
                if ($per_val >= 20) {
                    $per_val_color = "green";
                } else {
            ...
            if ($tot_quotaactual_mtd > 0 && $revenue_sales_b2b > 0) {
                //$per_val = number_format(str_replace(",", "", number_format($tot_quotaactual_mtd, 0)) * 100 / str_replace(",", "", number_format($revenue_sales_b2b, 0)), 2);
                $tot_quotaactual_mtd_numeric = (float)str_replace(",", "", number_format($tot_quotaactual_mtd, 0));
                $revenue_sales_b2b_numeric = (float)str_replace(",", "", number_format($revenue_sales_b2b, 0));

                // Check to avoid division by zero
                if ($revenue_sales_b2b_numeric != 0) {
                    $per_val = number_format($tot_quotaactual_mtd_numeric * 100 / $revenue_sales_b2b_numeric, 2);
                }
                if ($per_val >= 20) {
                    $per_val_color = "green";
                } else {
            ...
            if ($ttylyesno == "ttylyes") {
                ...
                echo "<span id='887" . $unqid . $MGArraytmp2["empid"] . "' style='display:none;'><a href='#' onclick='close_div(); return false;'>Close</a>" . ($lisoftrans_ttly_b2b_rev) . "</span>";
        ...
        if ($headtxt == "PALLET Leaderboard") {
            ...
            echo "</font></td>";
            echo "<td bgColor='$tbl_color' style='border-right-style:solid; border-right-width: thin; border-right-color: black;' align = 'right'><font color='" . $color . "'>";
            if ((($monthly_qtd_b2b_rev) > 0 || $quota_ov_b2b_rev > 0)) {
                if ($headtxt == "LAST WEEK" || $headtxt == "LAST MONTH" || $headtxt == "LAST QUARTER" || $headtxt == "LAST YEAR") {
                    echo number_format($revenue_sales_b2b * 100 / $quota_ov_b2b_rev, 2);
                } else {
                    //echo number_format($revenue_sales_b2b*100/$tot_quota_mtd_TD,2);
    ...
    function leadertbl_ucbzw(
        ...
        while ($rowemp = array_shift($result)) {
            ...
            while ($rowData = array_shift($resData)) {
                ...
                db();
                $popupRevenueContent .= "<tr><td><a target='_blank' href='viewCompany_func_water-mysqli.php?ID=" . $rowCompDtls["ID"] . "&show=watertransactions&company_id=" . $rowData["company_id"] . "&rec_type=&proc=View&searchcrit=&id=" . $rowData["company_id"] . "&b2bid=" . $rowCompDtls["ID"] . "&rec_id=" . $rowData["rec_id"] . "&display=water_sort#watersort'>" . $rowData['invoice_number'] . "</a></td><td>" . $vendor_name . "</td><td>" . $rowCompDtls['nickname'] . "</td><td align='right'>$" . number_format($rowData['amount'], 0) . "</td></tr>";
                $ucbzw_popupRevenueContent .= "<tr><td><a target='_blank' href='viewCompany_func_water-mysqli.php?ID=" . $rowCompDtls["ID"] . "&show=watertransactions&company_id=" . $rowData["company_id"] . "&rec_type=&proc=View&searchcrit=&id=" . $rowData["company_id"] . "&b2bid=" . $rowCompDtls["ID"] . "&rec_id=" . $rowData["rec_id"] . "&display=water_sort#watersort'>" . $rowData['invoice_number'] . "</a></td><td>" . $vendor_name . "</td><td>" . $rowCompDtls['nickname'] . "</td><td align='right'>$" . number_format($rowData['amount'], 0) . "</td></tr>";
            }
            ...
            $MGArray[] = array(
                ...
                'percent_val' => $color,
                'rev_lastyr_tilldt' => $rev_lastyr_tilldt,
                'lisoftrans' => $lisoftrans . "<br>" . $popupRevenueContent,
                'lisoftrans_ttly' => $lisoftrans_ttly,
                'lisoftrans_lastyear' => $lisoftrans_lastyear,
                'ucbzw_lisoftrans' => $ucbzw_lisoftrans,
                'ucbzw_popupRevenueContent' => $ucbzw_popupRevenueContent,
                'ucbzw_lisoftrans_ttly' => $ucbzw_lisoftrans_ttly,
                'ucbzw_lisoftrans_lastyear' => $ucbzw_lisoftrans_lastyear
            );
        }
        ...
        foreach ($MGArray as $MGArraytmp2) {
            ...
            $tot_quota_mtd_TD = $tot_quota_mtd_TD + round($monthly_qtd_TD, 0);
            $tot_quotaactual_mtd = $tot_quotaactual_mtd + round($summtd_SUMPO, 0);
            $tot_quota_deal_mtd = $tot_quota_deal_mtd + round((float)$monthly_deal_qtd, 0);
            $tot_rev_lastyr_tilldt = $tot_rev_lastyr_tilldt + round($MGArraytmp2["rev_lastyr_tilldt"], 0);

            $tot_creditAmount = $tot_creditAmount + round($MGArraytmp2["creditAmount"], 0);
    ...
    function leadertbl_GMI(
        ...
        $monthly_deal_qtd = $monthly_deal_qtd ?? 0;
        $monthly_deal_qtd = number_format($monthly_deal_qtd, 0);

        $tot_quota_mtd = $tot_quota_mtd + $monthly_qtd;
        $tot_quota_mtd_TD = $tot_quota_mtd_TD + $monthly_qtd_TD;
        $tot_quotaactual_mtd = $tot_quotaactual_mtd + $summtd_SUMPO;
        $tot_quota_deal_mtd = $tot_quota_deal_mtd + (float)$monthly_deal_qtd;
        ...
        $global_final_tot_quota_to_dt = $global_final_tot_quota_to_dt + ($monthly_qtd);
        ...
        if ($headtxt == "THIS MONTH" || $headtxt == "LAST MONTH") {
            $selData = "SELECT water_transaction.id as rec_id, vendor_id, water_transaction.make_receive_payment, water_transaction.made_payment, water_transaction.vendor_credit, water_transaction.invoice_number, water_transaction.company_id, water_transaction.amount  FROM water_transaction WHERE transaction_date BETWEEN '" . $start_Dt . " 00:00:00' AND '" . $end_Dt . " 23:59:59' and make_receive_payment = 1 and made_payment = 1 and water_transaction.amount > 0 ";
            //echo $selData . "<br>";
            db();
            $resData = db_query($selData);
            $totalCnt = tep_db_num_rows($resData);
        ...
        if ($ttylyesno == "ttylyes") {
            ...
            while ($rowData = array_shift($resData)) {
                ...
                $popupRevenueContent_ttly .= "<tr><td><a target='_blank' href='viewCompany_func_water-mysqli.php?ID=" . $rowCompDtls["ID"] . "&show=watertransactions&company_id=" . $rowData["company_id"] . "&rec_type=&proc=View&searchcrit=&id=" . $rowData["company_id"] . "&b2bid=" . $rowCompDtls["ID"] . "&rec_id=" . $rowData["rec_id"] . "&display=water_sort#watersort'>" . $rowData['invoice_number'] . "</a></td><td>" . $vendor_name . "</td><td>" . $rowCompDtls['nickname'] . "</td><td align='right'>$" . number_format($rowData['amount'], 0) . "</td></tr>";
            }
    ...
    function leadertbl_stretch(
        ...
        if ($summtd_SUMPO > 0) {
			...
			<td bgColor='#ABC5DF' align='right'>$" . number_format($summtd_SUMPO, 0) . "</td>
			<td bgColor='#ABC5DF' align='right'>" . number_format((float)str_replace(",", "", number_format($summtd_SUMPO, 0)) * 100 / (float)str_replace(",", "", number_format($revenue_sales_b2b_str, 0)), 0) . "%</td>
        ...
        if ($headtxt == "GMI Leaderboard") {
            ...
            $tot_quotaactual_mtd_numeric = (float)str_replace(",", "", number_format($tot_quotaactual_mtd, 0));
            $revenue_sales_b2b_str_numeric = (float)str_replace(",", "", number_format($revenue_sales_b2b_str, 0));

            if ($revenue_sales_b2b_str_numeric > 0) {
                $per_val = number_format(($tot_quotaactual_mtd_numeric * 100) / $revenue_sales_b2b_str_numeric, 2);
            }
            if ($per_val >= 20) {
                $per_val_color = "green";
            } else {
	...
	ORDER BY (sumsales_po_amunt) DESC";*/
        ...
        $sql = "SELECT count(*) as cnt, sum(round(loop_transaction_buyer.po_poorderamount,0)) as sumsales_po_amunt , loop_employees.id, loop_employees.activity_tracker_flg_pallet, loop_employees.status, loop_employees.name, loop_employees.initials, loop_employees.b2b_id FROM loop_transaction_buyer right join loop_employees on loop_employees.initials = loop_transaction_buyer.po_employee WHERE (loop_transaction_buyer.transaction_date BETWEEN '" . $start_Dt . "' AND '" . $end_Dt . " 23:59:59') and loop_transaction_buyer.ignore = 0 and loop_transaction_buyer.Leaderboard = 'PALLETS' and loop_employees.activity_tracker_flg_pallet = 1 and loop_employees.status = 'Active' group by loop_employees.id ORDER BY (sumsales_po_amunt) DESC";
        ...
        while ($rowemp = array_shift($result)) {
            ...
            $summtd_SUMPO = 0;
            db_b2b();
            $result_crm = db_query("Select sum(leads) as leads, sum(daily_touches) as daily_touches, sum(daily_quotes) as daily_quotes, sum(daily_deals) as daily_deals, sum(email_sent) as email_sent, sum(calls_made) as calls_made, sum(demand_entries) as demand_entries, sum(quote_requests) as quote_requests, sum(first_time_customer) as first_time_customer, sum(sales_po_amunt) as sales_po_amunt, sum(first_time_supplier) as first_time_supplier, sum(purchase_orders) as purchase_orders, sum(po_total) as po_total from employee_all_activity_details where entry_date BETWEEN '" . $start_Dt . "' AND '" . $end_Dt . " 23:59:59' and employee_id = '" . $rowemp["id"] . "'");
            while ($rowemp_crm = array_shift($result_crm)) {
            ...
            db();
            $result_crm = db_query("SELECT id, transaction_date FROM loop_transaction_buyer WHERE id in (SELECT min(id) FROM loop_transaction_buyer group by warehouse_id) and po_employee LIKE '" . $rowemp["initials"] . "' and loop_transaction_buyer.leaderboard = 'PALLETS' and loop_transaction_buyer.ignore < 1 AND transaction_date between '" . $start_Dt . "' AND '" . $end_Dt . " 23:59:59'");
            while ($rowemp_crm = array_shift($result_crm)) {
		...
		*/
        ...
        if ($emp_id_list_str != "") {
            ...
            $sql = "SELECT 0 as cnt, 0 as sumsales_po_amunt , loop_employees.id, loop_employees.activity_tracker_flg_pallet, loop_employees.status, loop_employees.name, loop_employees.initials, loop_employees.b2b_id FROM loop_employees WHERE loop_employees.activity_tracker_flg_pallet = 1 and loop_employees.status = 'Active' and id not in ($emp_id_list_str) ORDER BY loop_employees.name ";
        } else {
            $sql = "SELECT 0 as cnt, 0 as sumsales_po_amunt , loop_employees.id, loop_employees.activity_tracker_flg_pallet, loop_employees.status, loop_employees.name, loop_employees.initials, loop_employees.b2b_id FROM loop_employees WHERE loop_employees.activity_tracker_flg_pallet = 1 and loop_employees.status = 'Active' ORDER BY loop_employees.name ";
        }
        //echo $sql . "<br>";
        db();
        $result = db_query($sql);
        while ($rowemp = array_shift($result)) {
            ...
            $monthly_deal_qtd = $rowemp["cnt"];
            db();
            $result_crm = db_query("SELECT id, transaction_date FROM loop_transaction_buyer WHERE id in (SELECT min(id) FROM loop_transaction_buyer group by warehouse_id) and po_employee LIKE '" . $rowemp["initials"] . "' and loop_transaction_buyer.ignore < 1 AND transaction_date between '" . $start_Dt . "' AND '" . $end_Dt . " 23:59:59'");
            while ($rowemp_crm = array_shift($result_crm)) {
                $first_time_rec = $first_time_rec + 1;
            }
	...
	ORDER BY (sumpo_total) DESC";
        ...
        while ($rowemp = array_shift($result)) {
            ...
            $result_crm = db_query("Select sum(leads) as leads, sum(email_sent) as email_sent, sum(calls_made) as calls_made, sum(quote_requests) as quote_requests, sum(first_time_customer) as first_time_customer, sum(sales_po_amunt) as sales_po_amunt, sum(first_time_supplier) as first_time_supplier, sum(purchase_orders) as purchase_orders,  sum(po_total) as po_total from employee_all_activity_details where entry_date BETWEEN '" . $start_Dt . "' AND '" . $end_Dt . " 23:59:59' and employee_id = '" . $rowemp["id"] . "'");
            while ($rowemp_crm = array_shift($result_crm)) {
                $lead_tmp = $rowemp_crm["leads"];
                $contact_act_tmp = $rowemp_crm["email_sent"];
                $contact_act_ph1 = $rowemp_crm["calls_made"];
            ...
            db();
            $result_crm = db_query("SELECT 1 as cnt, STR_TO_DATE(loop_boxes_sort.sort_date, '%m/%d/%Y') as sort_date FROM loop_transaction inner join loop_boxes_sort on loop_boxes_sort.trans_rec_id = loop_transaction.id inner join loop_boxes on loop_boxes.id = loop_boxes_sort.box_id WHERE loop_transaction.id in (SELECT min(id) FROM loop_transaction group by warehouse_id) and loop_transaction.ignore < 1 and loop_boxes.owner = '" . $rowemp["id"] . "' AND STR_TO_DATE(loop_boxes_sort.sort_date, '%m/%d/%Y') between '" . $start_Dt . "' AND '" . $end_Dt . " 23:59:59' group by loop_boxes_sort.trans_rec_id");
            while ($rowemp_crm = array_shift($result_crm)) {
                $first_time_rec = $first_time_rec + $rowemp_crm["cnt"];
            }
        ...
        if ($emp_id_list_str != "") {
            $sql = "SELECT 0 as cnt, 0 as sumpo_total , employees.loopID as id, employees.activity_tracker_flg_purchasing_pallet, employees.status, employees.name, employees.initials, employees.employeeID as b2b_id FROM employees WHERE employees.activity_tracker_flg_purchasing_pallet = 1 and employees.status = 'Active' and employees.employeeID not in ($emp_id_list_str) group by employees.loopID ORDER BY name";
        } else {
            $sql = "SELECT 0 as cnt, 0 as sumpo_total , employees.loopID as id, employees.activity_tracker_flg_purchasing_pallet, employees.status, employees.name, employees.initials, employees.employeeID as b2b_id FROM employees WHERE employees.activity_tracker_flg_purchasing_pallet = 1 and employees.status = 'Active' group by employees.loopID ORDER BY name";
        }
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <input type="text" name="date_from" id="date_from" size="10"
                                                    value="<?php echo ($_GET['date_from']) ? $_GET['date_from'] : ''; ?>">
                                                <a href="#"
                                                ...
                                                <input type="text" name="date_to" id="date_to" size="10"
                                                    value="<?php echo ($_GET['date_to']) ? $_GET['date_to'] : ''; ?>">
                                                <a href="#"
                                        ...
                                        <tbody>
                                            ...
                                            while ($row = array_shift($result)) {
                                                ...
                                                if ($row["po_poorderamount"] > 0) {
                                                    echo "<tr><td bgColor='#E4EAEB' align='left' width='240px'><a target='_blank' href='search_results.php?warehouse_id=" . $row["warehouse_id"] . "&rec_type=Supplier&proc=View&searchcrit=&id=" . $row["warehouse_id"] . "&rec_id=" . $row["I"] . "&display=buyer_view'>" . $nickname . "</a></td>
                                    ...
                                    <table width="520" cellSpacing="1" cellPadding="1" border="0" id="table8"
                                        ...
                                        <tbody>
                                            ...
                                            if ($in_dt_range != "yes") {
                                                ...
                                                // }
                                                $date_from_timestamp = strtotime($date_from_val);

                                                if ($date_from_timestamp !== false) {
                                                    if (date('w', $date_from_timestamp) == 1) { // Check if the day is Monday
                                                        $yesterday = date('Y-m-d', strtotime($date_from_val . " -3 day"));
                                                    }
                                                }
                                                ...
                                                if ($date_from_timestamp !== false) {
                                                    if (
                                                        date('w', $date_from_timestamp) == 0 ||
                                                        date('w', $date_from_timestamp) == 2 ||
                                                        date('w', $date_from_timestamp) == 3 ||
                                                        date('w', $date_from_timestamp) == 4 ||
                                                        date('w', $date_from_timestamp) == 5 ||
                                                        date('w', $date_from_timestamp) == 6
                                                    ) {
                                            ...
                                            while ($row = array_shift($result)) {
                                                ...
                                                if ($row["po_poorderamount"] > 0) {
                                                    echo "<tr><td bgColor='#E4EAEB' align='left' width='240px'><a href='search_results.php?warehouse_id=" . $row["warehouse_id"] . "&rec_type=Supplier&proc=View&searchcrit=&id=" . $row["warehouse_id"] . "&rec_id=" . $row["I"] . "&display=buyer_view'>" . $nickname . "</a></td>
						<td width='20px' align='left' bgColor='#E4EAEB'>" . $po_delivery_dt . "</td><td width='20px' align='left'  bgColor='#E4EAEB'>" . $row["po_employee"] . "</td><td width='40px' align='right' bgColor='#E4EAEB'>$" . number_format($row["po_poorderamount"], 2) . "</td></tr>";
                                                }
                                                $po_poorderamount_tot = $po_poorderamount_tot + $row["po_poorderamount"];
                                            }
                                                ...
                                                <table>
                                                    <tr>
                                                        <td>
                                                            ...
                                                            activity_tracker($st_date, $end_date, "THIS WEEK", 'Y', 'Y', '#ABC5DF', '#E4EAEB', 'yes', $unqid, "ttylno");
                                                            ?>
                                                        </td>
                                                        <td width="50">&nbsp;</td>
                                                        <td>
                                                            ...
                                                            activity_tracker($st_friday_last, $st_thursday_last, "LAST WEEK", 'N', 'N', '#ABC5DF', '#E4EAEB', 'yes', $unqid, "ttylno");
                                                            ?>
                                                        </td>
                                            </td>
                                        </tr>
                            ...
                            <tr>
                                <td>
                                    ...
                                    if ($in_dt_range != "yes") {
                                        ...
                                        if ($date_from_timestamp !== false) {
                                            $st_date = date('Y-m-01', $date_from_timestamp); // First day of the month
                                            $end_date = date('Y-m-t', $date_from_timestamp); // Last day of the month
                                        }
                            ...
                            <tr>
                                <td>
                                    ...
                                    if ($in_dt_range != "yes") {
                                        ...
                                        if ($date_from_timestamp !== false) {
                                            $year = date('Y', $date_from_timestamp);
                                        }
                                    }
                            ...
                            <tr>
                                <td>
                                    ...
                                    if ($in_dt_range != "yes") {
                                        ...
                                        if ($date_from_timestamp !== false) {
                                            $st_date = date('Y-01-01', $date_from_timestamp);
                                            $end_date = date('Y-12-31', $date_from_timestamp);
                                        }
                            ...
                            <tbody>
                                ...
                                while ($rowemp = array_shift($result)) {
                                    ...
                                    if ($in_dt_range != "yes") {
                                        ...
                                        $sqlY = "SELECT ID, type, EmailID FROM CRM WHERE duplicate_added_by_system = 0 and type in ('phone', 'email') and EMPLOYEE LIKE '" . $rowemp["EMPLOYEE"] . "' AND  timestamp BETWEEN " . (strtotime($date_from_val . " -1 days") !== false ? Date("Y-m-d 00:00:00", strtotime($date_from_val . " -1 days")) : '1970-01-01 00:00:00') . " AND " . (strtotime($date_from_val) !== false ? Date("Y-m-d 00:00:00", strtotime($date_from_val)) : '1970-01-01 00:00:00');
                                    }
                                    ...
                                    if ($in_dt_range != "yes") {
                                        ...
                                        $date_from_timestamp = strtotime($date_from_val);
                                        $date_minus_1_day = strtotime($date_from_val . " -1 days");

                                        if ($date_from_timestamp !== false && $date_minus_1_day !== false) {
                                            $result_crm = db_query("SELECT count(unqid) as cnt FROM tblemail WHERE emaildate BETWEEN '" . date("Y-m-d 00:00:00", $date_minus_1_day) . "' AND '" . date("Y-m-d 00:00:00", $date_from_timestamp) . "' AND fromadd = '" . $rowemp["email"] . "'");
                                        }
                                    }
                                    while ($rowemp_crm = array_shift($result_crm)) {
                                        $contact_act_tmp = $contact_act_tmp + $rowemp_crm["cnt"];
                                    ...
                                    if ($in_dt_range != "yes") {
                                        ...
                                        $date_from_timestamp = strtotime($date_from_val);
                                        $date_minus_7_days = strtotime($date_from_val . " -7 days");

                                        if ($date_from_timestamp !== false && $date_minus_7_days !== false) {
                                            $sql7 = "SELECT ID, type, EmailID FROM CRM WHERE duplicate_added_by_system = 0 AND type IN ('phone', 'email') AND EMPLOYEE LIKE '" . $rowemp["EMPLOYEE"] . "' AND timestamp BETWEEN '" . date("Y-m-d 00:00:00", $date_minus_7_days) . "' AND '" . date("Y-m-d 00:00:00", $date_from_timestamp) . "'";
                                        }
                                    }
                                    //$result_crm = db_query($sql7 );
                                    ...
                                    if ($in_dt_range != "yes") {
                                        ...
                                        if ($date_from_timestamp !== false && $date_minus_7_days !== false) {
                                            $result_crm = db_query("SELECT count(unqid) as cnt FROM tblemail WHERE emaildate BETWEEN '" . date("Y-m-d 00:00:00", $date_minus_7_days) . "' AND '" . date("Y-m-d 00:00:00", $date_from_timestamp) . "' AND fromadd = '" . $rowemp["email"] . "'");
                                        }
                                    }
                                    while ($rowemp_crm = array_shift($result_crm)) {
                                        $contact_act_tmp = $contact_act_tmp + $rowemp_crm["cnt"];
                                    ...
                                    if ($in_dt_range != "yes") {
                                        ...
                                        $date_from_timestamp = strtotime($date_from_val);
                                        $date_minus_30_days = strtotime($date_from_val . " -30 days");

                                        if ($date_from_timestamp !== false && $date_minus_30_days !== false) {
                                            $sql30 = "SELECT ID, type, EmailID FROM CRM WHERE duplicate_added_by_system = 0 AND type IN ('phone', 'email') AND EMPLOYEE LIKE '" . $rowemp["EMPLOYEE"] . "' AND timestamp BETWEEN '" . date("Y-m-d 00:00:00", $date_minus_30_days) . "' AND '" . date("Y-m-d 00:00:00", $date_from_timestamp) . "'";
                                        }
                                    }
                                    ...
                                    if ($in_dt_range != "yes") {
                                        ...
                                        if ($date_from_timestamp !== false && $date_minus_30_days !== false) {
                                            $result_crm = db_query("SELECT count(unqid) as cnt FROM tblemail WHERE emaildate BETWEEN '" . date("Y-m-d 00:00:00", $date_minus_30_days) . "' AND '" . date("Y-m-d 00:00:00", $date_from_timestamp) . "' AND fromadd = '" . $rowemp["email"] . "'");
                                        }
                                    }
                                    while ($rowemp_crm = array_shift($result_crm)) {
                                        $contact_act_tmp = $contact_act_tmp + $rowemp_crm["cnt"];
                                    ...
                                    if ($in_dt_range != "yes") {
                                        ...
                                        if ($date_from_timestamp !== false && $date_minus_7_days !== false) {
                                            $sql7 = "SELECT * FROM quote WHERE rep LIKE '" . $b2bempid . "' AND qstatus != 2 AND quoteDate BETWEEN '" . date("Y-m-d 00:00:00", $date_minus_7_days) . "' AND '" . date("Y-m-d 00:00:00", $date_from_timestamp) . "'";
                                        }
                                    }
                                    $result_new = db_query($sql7);
                                    ...
                                    if ($in_dt_range != "yes") {
                                        ...
                                        if ($date_from_timestamp !== false && $date_minus_30_days !== false) {
                                            $sql30 = "SELECT * FROM quote WHERE rep LIKE '" . $b2bempid . "'  AND qstatus != 2 AND quoteDate BETWEEN '" . date("Y-m-d 00:00:00", $date_minus_30_days) . "' AND '" . date("Y-m-d 00:00:00", $date_from_timestamp) . "'";
                                        }
                                    }
            ...
            <tr>
                <td>
                    <?php
                            ...
                            activity_tracker($date_from_val, $date_to_val, "Search", 'Y', 'Y', '#ABC5DF', '#E4EAEB', 'yes', $unqid, "ttylno");
                    ?>
                </td>
            </tr>
        </table>
