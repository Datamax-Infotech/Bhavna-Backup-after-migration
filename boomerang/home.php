...
<div class="container-fluid main_section footer-top">
    <div class="px-5">
        <div class="row justify-content-between">
            ...
            <!-- <div class="col-md-3">
                <h5>CONTACT</h5>
                <p>Account Manager: Donna Molina</p>
                <p>Phone: 323.724.2500 x754 </p>
                <p>Email: <a class="color_173D6E" href="mailto:Boomerang@UsedCardboardBoxes.com">Boomerang@UsedCardboardBoxes.com</a></p>
            ...
            <?php
            // Assuming db() function establishes a database connection
            db();

            // Retrieve login ID from the cookie
            $loginid = isset($_COOKIE['loginid']) ? $_COOKIE['loginid'] : "";

            if ($loginid) {
                // Fetch user's email and phone from boomerang_usermaster
                $user_query = "SELECT user_email, user_phone, manager_id FROM boomerang_usermaster WHERE loginid = '" . $loginid . "'";
                $user_result = db_query($user_query);
                $user_data = array_shift($user_result); // Get the first row

                // Check if user data was found
                if ($user_data) {
                    $user_email = htmlspecialchars($user_data['user_email']);
                    $user_phone = htmlspecialchars($user_data['user_phone']);
                    $manager_id = $user_data['manager_id'];
                    db();
                    // Fetch manager's name from employees table using the manager_id
                    $manager_query = "SELECT username FROM loop_employees WHERE id = '" . $manager_id . "'";
                    $manager_result = db_query($manager_query);
                    $manager_data = array_shift($manager_result); // Get the first row

                    // Check if manager data was found
                    if ($manager_data) {
                        $manager_name = htmlspecialchars($manager_data['username']);
                    } else {
                        $manager_name = "N/A"; // Default if no manager found
                    }
                } else {
                ...
                // Default values if no login ID found in cookie
                $user_email = "N/A";
                $user_phone = "N/A";
                $manager_name = "N/A";
            }
            ...
            <div class="col-md-3">
                <h5>CONTACT</h5>
                <p>Account Manager: <?php echo $manager_name; ?></p>
                <p>Phone: <?php echo $user_phone; ?></p>
                <p>Email: <a class="color_173D6E" href="mailto:<?php echo $user_email; ?>"><?php echo $user_email; ?></a></p>
                <h5 class="mt-2">UCB Environment Companies</h5>
                <ul>
                    <li><a class="color_FFF" href="https://www.usedcardboardboxes.com/">- UsedCardboardBoxes.com</a></li>
                    <li><a class="color_FFF" href="https://www.ucbzerowaste.com/">- UCBZeroWaste</a></li>
