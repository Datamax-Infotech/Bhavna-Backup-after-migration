...
$inv_b2b_id_list_arr = array();
if ($_REQUEST["inv_b2b_id_list"] != "") {
	$inv_b2b_id_list_arr = explode(",", $_REQUEST["inv_b2b_id_list"]);
}

foreach ($inv_b2b_id_list_arr as $inv_b2b_id_list_arr_tmp) {
	$box_inv_id = $inv_b2b_id_list_arr_tmp;
	$sql_data = "Select inventory.loops_id, inventory.ID, inventory.vendor_b2b_rescue, inventory.description, inventory.quantity, tmp_inventory_list_set2.sales_order_qty, inventory.actual_qty_calculated from inventory 
		INNER JOIN tmp_inventory_list_set2 ON inventory.loops_id = tmp_inventory_list_set2.trans_id Where loops_id = '" . $box_inv_id . "'";
	db_b2b();
	...
	while ($myrowsel_b2b = array_shift($dt_view_res_data)) {
		...
		//$box_qty = $box_qty + str_replace(",", "" ,$myrowsel_b2b["quantity"]);

		$actual_qty_calculated = $myrowsel_b2b["actual_qty_calculated"];
		$sales_order_qty = $myrowsel_b2b["sales_order_qty"];

		if ($actual_qty_calculated != 0 or $actual_qty_calculated - $sales_order_qty != 0) {
			$box_qty = $box_qty + ($actual_qty_calculated - $sales_order_qty);
		}
		...
		$box_weight = $box_weight + ($myrowsel_b2b["quantity"] * $bweight);
		//echo "Quantity : " . $myrowsel_b2b["quantity"] . "=" . $bweight . "=" . $box_weight . "<br>";
	}
}
...
if ($nowarehouse == "no") {
	...
	if ($ifany_err == "") {
		...
		//$avg_quote_amount_tot_org = str_replace(",", "", $avg_quote_amount_tot_org);
		$avg_quote_amount_tot = number_format($quote_amount_tot * 1.01, 2);

		//$finalval = $avg_quote_amount_tot_org / $box_qty;
		$avg_quote_amount_tot_org = (float)$avg_quote_amount_tot_org;
		$box_qty = (float)$box_qty;

		$finalval = $avg_quote_amount_tot_org / $box_qty;
		//echo $avg_quote_amount_tot_org . " = " . $box_qty . "<br>";
		echo "<table >
				 <tr><td style='border: 1px solid #000;' valign=top><font size='4'><b>$pickup_nm</b></font></td></tr>
				 <tr><td style='border-bottom: 1px solid #000; border-left: 1px solid #000; border-right: 1px solid #000;' ><b>$" . $avg_quote_amount_tot . "</b>/$" . number_format($finalval, 2) . " each</td></tr>
				</table>";
	}
}
?>