...
if (isset($_REQUEST['form_action']) && $_REQUEST['form_action'] == "update_address") {
	...
	$update_qry = "UPDATE boomerang_user_addresses 
                   SET first_name = '" . $_REQUEST['first_name'] . "', 
                       ...
                       dock_hours = '" . $_REQUEST['dock_hours'] . "' 
                   WHERE id = '" . $_REQUEST['address_id'] . "'";
	db_query($update_qry);

	// If the checkbox "Set as default" is checked, update the default address
	if (isset($_REQUEST['add_set_as_def'])) {
		// Set all other addresses for this user to "not default"
		$user_id = $_COOKIE['loginid'];
		$reset_default_qry = "UPDATE boomerang_user_addresses SET mark_default = 0 WHERE user_id = '" . $user_id . "'";
		db_query($reset_default_qry);

		// Set the current address as default
		$set_default_qry = "UPDATE boomerang_user_addresses SET mark_default = 1 WHERE id = '" . $_REQUEST['address_id'] . "'";
		db_query($set_default_qry);
	}
