				...
				if ($myrowsel = array_shift($result)) {
					...
					?>
						...
						<TD CLASS='<?php echo $shade; ?>'>
							<a target="_blank" href="orders.php?tbl=<?php echo encrypt_url($_REQUEST["tbl"]); ?>&id=<?php echo encrypt_url($orders_id); ?>&proc=View"><?php echo $orders_id; ?></a>
						</TD>
					...
					if ($_REQUEST['delete'] == "yes") {
						...
						if (empty($result)) {
							if (!headers_sent()) {    //If headers not sent yet... then do php redirect
								header('Location: shipments_shipped.php?posting=yes&');
								exit;
							} else {
