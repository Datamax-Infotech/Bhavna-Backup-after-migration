...
if ($rec_found == "yes") {
	...
	$sql = "Update ucbdb_employees_log_attempt set attempt = 0 WHERE user_name = ?";
	$result = db_query($sql, array("s"), array($myusername));
	if($ublock == 1){
		$q1 = "UPDATE $tbl_name SET user_block=0, user_block_time=NULL WHERE id=?";
		db_query($q1, array("i"), array($employeeid));
	}
	...
	$sql = "SELECT * FROM $tbl_name WHERE email=? AND status = 'Active' ";
	$result = db_query($sql , array("s"), array($myusername));
	while ($rq = array_shift($result)) {
		$user_found = "yes";
	...
	if ($user_found == "yes"){
		...
		$resAttemptDt = db_query("SELECT attempt FROM ucbdb_employees_log_attempt WHERE user_name = ? AND login_datetime = '" . date("Y-m-d") . "' ", array("s"), array($myusername) );
		$rowAttemptDt = array_shift($resAttemptDt);
		if(!empty($rowAttemptDt)){
			$attempt = $rowAttemptDt['attempt'] + 1;
			db_query("UPDATE ucbdb_employees_log_attempt SET attempt = '".$attempt."' WHERE user_name = ? AND login_datetime = '" . date("Y-m-d") . "' ", array("s"), array($myusername));
		}else{
			$attempt = 1;
		...
		if($user_block == 0){
			...
			if($attempt >= 3){
				db_query("UPDATE $tbl_name SET user_block = 1, user_block_time  = '".date("Y-m-d H:i:s") ."' WHERE email = '".$myusername."' ");
			}
			$attempt = 3 - $attempt;
