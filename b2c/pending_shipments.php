			...
			if ($posting == "yes") {
				...
				if ($myrowsel = array_shift($result)) {
					...
					do {
						...
						<TD CLASS='<?php echo $shade; ?>'>
							<a target="_blank"
								href="orders.php?tbl=<?php echo $tbl_nm; ?>&id=<?php echo encrypt_url($orders_id); ?>&proc=View">
								<?php echo $orders_id; ?>
							</a>
						</TD>
						...
						<?php } else { ?>
							<TD CLASS='<?php echo $shade; ?>'>
								<a
									href="<?php echo $thispage; ?>?tbl=<?php echo $tbl_nm; ?>&id=<?php echo encrypt_url($id); ?>&proc=Delete&<?php echo $pagevars; ?>&orders_id=<?php echo $orders_id; ?>">Remove</a>
							</TD>
						<?php } ?>
						</TR>
						<?php
		...
		if ($proc == "Delete") {
			...
			if (!$delete) {
				...
				<DIV CLASS='PAGE_OPTIONS'>
					...
					<a
						href="<?php echo $thispage; ?>?tbl=<?php echo $tbl_nm; ?>&id=<?php echo encrypt_url($id); ?>&delete=yes&proc=Delete&orders_id=<?php echo $orders_id; ?>&<?php echo $pagevars; ?>">Yes</a>
					<a href="<?php echo $thispage; ?>?tbl=<?php echo $tbl_nm; ?>&<?php echo $pagevars; ?>">No</a>
				</DIV>
				<?php
			} //IF !DELETE
			...
			if ($delete == "yes") {
				...
				if (empty($result)) {
					if (!headers_sent()) {    //If headers not sent yet... then do php redirect
						header('Location: pending_shipments.php?posting=yes&');
						exit;
					} else {
						echo "<script type=\"text/javascript\">";
						echo "window.location.href=\"pending_shipments.php?tbl=" . $tbl_nm . "&posting=yes&\";";
						echo "</script>";
						echo "<noscript>";
						echo "<meta http-equiv=\"refresh\" content=\"0;url=pending_shipments.php?tbl=" . $tbl_nm . "&posting=yes&\" />";
						echo "</noscript>";
						exit;
					}
				} else {
